...
const Dashboard = () => {
  ...
  const projectStatusData = STATUS_ORDER.map((status) => ({
    ...
    value: calculatePercentage(
      Object.values(data[`total${status.replace(/\s/g, "")}Projects`]).reduce(
        (a, b) => a + b,
        0
      )
    ),
  ...
  const labCharts = labs.map((lab) => {
    ...
    const labData = STATUS_ORDER.map((status) => {
      const statusCount =
        data[`total${status.replace(/\s/g, "")}Projects`][lab] || 0;
      return {
        name: status,
            ...
            <PieChart width={300} height={200}>
              ...
              <Legend
                ...
                formatter={(value, entry) =>
                  `${value}: ${parseFloat(entry.payload.value).toFixed(2)}%`
                }
              />
            </PieChart>
          </Card.Body>
  ...
  return (
    <div className="dashboard">
      ...
      <Container
        ...
        id="export-pdf"
        className="mt-4"
        style={{ margin: "0px", marginLeft: "5%" }}
      >
        <h1 className="text-center mb-4">
          ForgeXperimental Projects Dashboard{" "}
        </h1>
        <Row className="mb-4">
          <Col md={4}>
            <Card className="shadow-sm">
                ...
                <Card.Body className="chart-content">
                  ...
                  <PieChart width={300} height={200}>
                    ...
                    <Legend
                      ...
                      wrapperStyle={{ paddingLeft: "10px" }}
                      formatter={(value, entry) =>
                        `${value}: ${parseFloat(entry.payload.value).toFixed(
                          2
                        )}%`
                      }
                    />
        ...
        <Row className="mb-4">{labCharts}</Row>
      </Container>
