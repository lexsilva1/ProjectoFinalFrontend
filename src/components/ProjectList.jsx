...
const ProjectList = ({ userId }) => {
  ...
  return (
    <div className="project-list-card"> 
      <h6><strong>{t("Projects")}:</strong></h6>
      <div>
        <select onChange={(e) => setSortType(e.target.value)} value={sortType} style={{ borderRadius: "5px", border: "1px solid #cbcbcb" }}>
          <option value="" disabled>{t("Sort by")}</option>
      ...
      {projectsDetails.map((project, index) => (
        <Link to={`/project/${project.name}`} style={{ textDecoration: 'none' }} key={index}>
