  ...
  const handleAnnotationChange = async (e) => {
    ...
    if (value.includes("#") && !/\S#\S/.test(value)) {
      ...
      try {
        ...
        const projectMembers = projectDetails.teamMembers.filter(
          (member) =>
                ...
                <div className="log-time">
                  ...
                  {log.userId && userDetails[log.userId] && (
                    <span style={{ marginLeft: "25px" }}>
                      {userDetails[log.userId].firstName}{" "}
                      {userDetails[log.userId].lastName}
                    </span>
                  )}
                </div>
                <div className="log-icon">{logLevelIcons[log.level]}</div>
                <h5>{log.message}</h5>
                <div>
                  {log.type === "OTHER"
                    ? log.log
                    : logTypeMessages[log.type] || log.type}
                </div>
              </Card.Header>
            </Card>
          ))
